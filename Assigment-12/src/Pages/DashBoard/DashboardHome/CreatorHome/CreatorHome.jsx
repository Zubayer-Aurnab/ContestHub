...
const CreatorHome = () => {
    ...
    const totalWinningPrice = myCreated?.reduce((total, item) => total + Number(item.price), 0)
    console.log(totalWinningPrice)
    const totalContest = AllContest?.length;
    const yourContest = myCreated?.length;
    const percentage = (yourContest / totalContest) * 100;

    const data = [
        { name: 'Total Contest', value: totalContest, percentage: 100 },
        { name: 'Your Added Contest', value: yourContest, percentage: percentage },
    ];
